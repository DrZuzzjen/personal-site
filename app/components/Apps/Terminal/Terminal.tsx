...
export default function Terminal({
	...
	// Debug activeEffect changes
	useEffect(() => {
		console.log('[DEBUG] Terminal activeEffect changed:', activeEffect);
	}, [activeEffect]);
	...
	useEffect(() => {
		if (!initializedRef.current) {
			...
			if (isMobile) {
				...
				appendLine({
					text: `${typeof navigator !== 'undefined' ? (navigator.userAgent.includes('iPhone') ? 'iPhone' : navigator.userAgent.includes('Android') ? 'Android' : 'Mobile device') : 'Mobile device'} detected`,
					type: 'warning',
				});
				appendLine({ text: '', type: 'output' });
				appendLine({
					text: '⚠️  Desktop UI disabled on mobile',
					type: 'error',
				});
				...
				appendLine({
					text: 'Welcome to Jean Francois Portfolio!',
					type: 'success',
				});
				...
				appendLine({
					text: 'Initializing MS-DOS compatible shell...',
					type: 'system',
				});
				appendLine({
	...
	const displayPath = useMemo(
		...
		[currentPath, formatPathForDisplay]
	);
