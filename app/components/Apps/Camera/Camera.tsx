...
export default function Camera({ onClose }: CameraProps) {
	...
	if (error) {
		return (
			...
			>
				<h3 style={{ color: COLORS.WIN_BLACK, marginBottom: 16 }}>
					⚠️ Camera Access Problem
				</h3>
	...
	return (
		...
		>
			...
			>
				...
				<button
					...
					style={{
						...
						color: COLORS.WIN_WHITE,
						border: `2px solid ${
							isActive ? COLORS.BORDER_LIGHT : COLORS.BORDER_SHADOW
						}`,
						cursor: isActive ? 'pointer' : 'not-allowed',
						borderRadius: 4,
						boxShadow: isActive
							? `2px 2px 0px ${COLORS.BORDER_SHADOW}`
							: 'none',
					}}
				>
					📸 Take Screenshot
			...
			>
				Status:{' '}
				{isActive
					? '📹 Camera active - Ready to take photos!'
					: '📴 Camera offline'}
			</div>
		</div>
	);
